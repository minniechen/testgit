...
def createNewTelusAccount(ENV, accountStatus, brandID, tierID, languageID, emailAddress, numberOfAssignedUsers, numberOfUnassignUsers, deviceID, DIDType):
    ...
    for i in range(1, int(numberOfAssignedUsers)+1):
        ...
        if deviceID != 'none':
            telus_rest_api.addDeviceForAassignedUser(ENV, accountMailboxID, assignUserMailboxID, int(deviceID))
        if DIDType != 'none':
    ...
    for j in range(1, int(numberOfUnassignUsers)+1):
        ...
        if deviceID != 'none':
            telus_rest_api.addDeviceForUnaassignedUser(ENV, accountMailboxID, unassignUserIDs[0], int(deviceID))
        if DIDType != 'none':
