...
def getBrandToken(ENV, brandID):
    baseUrl = "http://" + ENV + "/restapi/oauth/token"
    keyEndcode = 'Basic QUFDNTRhOGE5ZDRmYzIyNzQwQTdGMEIyZWYwQWYyMDc3ZGYzOWNlNDJGREFCOTAwZjM4ODAyNkI0MTJlYTg0RDpENWQwNkY2NEMwOGU5ODZiNDQxQUU0YjAxY2VjZDgzZWYxNjAzRTQ5MDVlMGM3N2Q0ZDdlYzUxOWZjQ0IxOEUw'
...
def getLocalNumbers(ENV, brandID):
    baseUrl = "http://" + ENV + '/restapi/v1.0/number-pool/lookup'
    payload = {'brandId': brandID, 'paymentType': 'Local', 'countryId': 39, 'perPage': 10, 'npa': 780}
...
def getTFNumbers(ENV, brandID):
    baseUrl = "http://" + ENV + '/restapi/v1.0/number-pool/lookup'
    payload = {'brandId': brandID, 'paymentType': 'TollFree', 'countryId': 39, 'perPage': 10, 'npa': 800}
    headers = {'Authorization': getBrandToken(ENV, brandID), 'Content-Type': 'application/json'}
    getLocalNumbersResponse = requests.post(baseUrl, params=payload, headers=headers)
    # print(getLocalNumbersResponse.text)
    data = getLocalNumbersResponse.json()
...
def createTelusAccount(ENV, brandID, tierID, languageID, emailAddress):
    baseUrl = "http://" + ENV + '/restapi/v1.0/account'
...
def unConfirmAccount(ENV, accountMailboxID):
    baseUrl = "http://" + ENV + '/restapi/v1.0/account/' + str(accountMailboxID)
    ...
    # print(getActivateAccountResponse.text)
...
def addAssignUser(ENV, accountMailboxID, emailAddress):
    baseUrl = "http://" + ENV + '/restapi/v1.0/account/' + str(accountMailboxID) + '/extension'
    dateTime = time.strftime('%Y%m%d%H%M%S', time.localtime())
    emailAddress = emailAddress.split('@')
    emailAddress = emailAddress[0] + '+' + dateTime  + str(random.randint(1,3)) + '@' + emailAddress[1]
    # print('time: ' + dateTime)
...
def addUnassignUser(ENV, accountMailboxID):
    ...
    if getUnassignUserResponse.ok:
        ...
        print('New unassign user is created, mailbox id: ' + unassignUserParterID)
...
def addPhoneNumberForAassignedUser(ENV, brandID, accountMailboxID, extensionMailboxID, DIDType):
    baseUrl = "http://" + ENV + '/restapi/v1.0/account/' + str(accountMailboxID) + '/phone-number'
    dateTime = time.strftime('%Y%m%d%H%M%S', time.localtime())
    # print('time: ' + dateTime)
...
def addDeviceForAassignedUser(ENV, accountMailboxID, extensionMailboxID, deviceID):
    baseUrl = "http://" + ENV + '/restapi/v1.0/account/' + str(accountMailboxID) + '/order'
    dateTime = time.strftime('%Y%m%d%H%M%S', time.localtime())
    # print('time: ' + dateTime)
    headers = {'Authorization': getAccountToken(ENV, accountMailboxID), 'Content-Type': 'application/json'}
    if deviceID == -1:
