    ...
    if accountID != '':
        ...
        if int(userMailboxID) != 0  and deviceID != 'none':
            if telus_rest_api.getExtensionInfo(ENV, accountID, userMailboxID)[1] == 'Unassigned':
                telus_rest_api.addDeviceForUnaassignedUser(ENV, accountID, userMailboxID, deviceID)
            else:
                telus_rest_api.addDeviceForAassignedUser(ENV, accountID, userMailboxID, deviceID)
        ...
        for i in range(1, int(numberOfAssignedUsers)+1):
            ...
            if deviceID != 'none':
                telus_rest_api.addDeviceForAassignedUser(ENV, accountID, assignUserMailboxID, deviceID)
            if DIDType != 'none':
        ...
        for j in range(1, int(numberOfUnassignUsers)+1):
            ...
            if deviceID != 'none':
                telus_rest_api.addDeviceForUnaassignedUser(ENV, accountID, unassignUserIDs[0], deviceID)
            if DIDType != 'none':
